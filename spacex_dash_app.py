...
def get_scatter(entered_site, range_payload):

    min_val, max_val = range_payload
    scatter_data=spacex_df[(spacex_df['Payload Mass (kg)'] > min_val) & (spacex_df['Payload Mass (kg)'] < max_val)]

    if entered_site == 'ALL':
        fig = px.scatter(
            scatter_data, x='Payload Mass (kg)', y='class',
            hover_data=['Booster Version'],
            color='Booster Version Category',
            title='Success launches for different Payload masses for all sites')
        return fig
    else:
        scatter_data = scatter_data[spacex_df['Launch Site'] == entered_site]
        fig=px.scatter(
            scatter_data,x='Payload Mass (kg)', y='class', 
            title = f'Success for {entered_site} per Payload Mass', 
            color='Booster Version Category')
        return fig
